...
4. **有用性 timeout 可收获 local 引理（`LEMMA_LIBRARY_LOCAL`，默认开）。** 有用性仍先跑；若 2s 内尚未返回，才用两路 CVC（`cvc5_inductive` + `cvc4_default`，Vampire 则顺序/并行直接证）投机 \(A \vdash c_i\)。`unsat` 时已证出的 \(c_i\) 标 `pin` 且不再 `prove_run` 开头 60s；未证出的 `skip_initial` 后递归。`timeout` 时只把已直接证出的公式以 `role=local` 入库，不递归，并再证当前 \(G\)。库满时 local 不能挤 pin。`paper.env` 关掉此开关。
...
库满（12）时：**local 不能挤 pin；新 pin 可以挤最旧 local。** 无 `role` 的旧记录视为 pin。`LEMMA_LIBRARY_LOCAL=off` 时仍用原来的 FIFO `lemmas[-12:]`。
