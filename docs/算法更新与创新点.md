...
4. **有用性 timeout 可收获 local 引理（`LEMMA_LIBRARY_LOCAL`，默认开）。** 有用性仍先跑；若 2s 内尚未返回，才用两路 CVC（`cvc5_inductive` + `cvc4_default`，Vampire 则顺序/并行直接证）投机 \(A \vdash c_i\)。`unsat` 时已证出的 \(c_i\) 标 `pin` 且不再 `prove_run` 开头 60s；未证出的 `skip_initial` 后递归。`timeout` 时只把已直接证出的公式以 `role=local` 入库，不递归，并用 `HARVEST_RETRY_TIMEOUT`（默认 2s，不是全局 60s）再证当前 \(G\)。库不按条数封顶。`paper.env` 关掉此开关。
5. **LLM 墙钟按剩余时间分配。** 根节点对齐 `TASK_TIMEOUT`。有用性默认对齐论文 60s。LLM 很少顶满时限，所以先把单次 LLM 从 `LLM_TIMEOUT` 压到 90s，HTTP 重试仍保留。若 90s×次数 + 60s 仍放不下，再把有用性从 60s 压到 2s。有用性再往下就丢掉额外 HTTP 重试并重算；最后才把 LLM 压到 90s 以下。有用性实际超时在 LLM 返回后再按剩余墙钟截断到 \([2,60]\)。不均分剩余 attempt。
