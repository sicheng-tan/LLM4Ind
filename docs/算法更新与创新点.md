...
| 引理库 `lemma_library.json` | 整题共享 | 已完全放电的定理（`pin`）以及 timeout 收获的 `local`，注入后续 SMT 公理 |
...
4. **筛选按成员剔除（`LEMMA_FILTER_DROP`，默认开）。** 与目标相同（含 α）、与题级库中已有公式 α 相同、未定义符号、1s 矛盾、已记录 invalid 都只丢掉那一条；组里还剩至少一条就对 \(C'\) 做有用性。撞库的成员不是 invalid（已是定理）。整组只因撞库被掏空时，用当前库短再证 \(G\)。`add_proved_lemma`（含 harvest local）对**当场活库**做 α 去重。`paper.env` 关回原文的「一人失败则整组作废」。
5. **有用性 timeout 可收获 local 引理（`LEMMA_LIBRARY_LOCAL`，默认开）。** 有用性仍先跑；若 2s 内尚未返回，才用两路 CVC（`cvc5_inductive` + `cvc4_default`，Vampire 则顺序/并行直接证）投机 \(A \vdash c_i\)。`unsat` 时已证出的 \(c_i\) 标 `pin` 且不再 `prove_run` 开头 60s；未证出的 `skip_initial` 后递归。`timeout` 时只把已直接证出的公式以 `role=local` 入库，不递归，并用 `HARVEST_RETRY_TIMEOUT`（默认 2s，不是全局 60s）再证当前 \(G\)。库不按条数封顶。`paper.env` 关掉此开关。
6. **LLM 墙钟按剩余时间分配。** 根节点对齐 `TASK_TIMEOUT`。有用性默认对齐论文 60s。LLM 很少顶满时限，所以先把单次 LLM 从 `LLM_TIMEOUT` 压到 90s，HTTP 重试仍保留。若 90s×次数 + 60s 仍放不下，再把有用性从 60s 压到 2s。有用性再往下就丢掉额外 HTTP 重试并重算；最后才把 LLM 压到 90s 以下。有用性实际超时在 LLM 返回后再按剩余墙钟截断到 \([2,60]\)。不均分剩余 attempt。
...
flowchart TD
  ...
  nonempty -->|是| screen["按成员筛选：same-as-goal / 已在库中 / 未定义符号 / 1s 矛盾"]
  screen -->|整组被掏空| invalid["记 invalid"]
  screen -->|仍有 C'| useful["60s 有用性：A ∪ Lib ∧ C' ⊢ G"]
...
flowchart TD
  ...
  family -->|只有重写族| rw["用 term_rewrite"]
  family -->|只有一般化 / 归纳族| eq["用 equational_reasoning"]
  family -->|两类都有| sample["P(term_rewrite)=rewrite/(rewrite+generalize)"]
  family -->|没有 hint| paper["论文顺序：先 equational"]
  eq --> run["本 attempt 生成"]
  rw --> run
  sample --> run
  paper --> run
...
`need_rewrite` 走 term_rewrite、`need_stronger_lemma` 走 equational，是按模板内容对齐的：前者教定向重写，后者教归纳步上的等式推理。实现见 `preferred_prompt_for_hints`。
...
1. **入库 = 整棵子树成功，或 timeout 路径上的直接证明。** 有用性 `unsat` 且子目标证出 → `role=pin`。有用性 `timeout` 且 \(A \vdash c_i\) 直接证出 → `role=local`（不递归）。中间节点失败时，只有已经 `proved` 的后代进库。`useful_but_unproved` 和仅有 progress 的公式都不是定理。
2. **参考树最多一棵，且必须是 `obligation_tree`。** 不把历次失败组、空 attempt 拼成森林塞进 prompt。local 第一版只进 `lemma_library.json`，不挂到义务树上的 `proved`（避免自动升 pin）。

库不按条数封顶。α 等价去重；无 `role` 的旧记录视为 pin；local 可就地升 pin。`LEMMA_LIBRARY_LOCAL=off` 时不写入 local，已有条目也不再 FIFO 截断。
...
LEMMA_FILTER_DROP       筛掉不合格成员后用剩余 C' 做有用性（默认开；paper 关则整组作废）
LEMMA_LIBRARY           已证公式入库并注入公理
LEMMA_LIBRARY_LOCAL     有用性 timeout 收获直接证出的引理为 local（默认开；paper 关）
HARVEST_RETRY_TIMEOUT   timeout 收获后短再证 \(G\) 的秒数（默认 2；不改 60s 有用性/初始预算）
